...
const OUTPUT_PATH = path.join(OUTPUT_DIR, "today-dashboard.json");
const CLEAN_OUTPUT_PATH = path.join(OUTPUT_DIR, "today-dashboard.cleaned.json");
const HTML_SNAPSHOT_PATH = path.join(OUTPUT_DIR, "today-dashboard.html");
const SCREENSHOT_PATH = path.join(OUTPUT_DIR, "today-dashboard.png");
...
async function waitForAuthenticationTransition(page, timeoutMs = 180000) {
...
async function waitForElectricNumberTransition(page, startUrl, startText, timeoutMs = 120000) {
  const deadline = Date.now() + timeoutMs;
...
async function ensureAuthenticated(context, page) {
  ...
  if (!(await waitForAuthenticationTransition(page))) {
    log("Authentication was not detected automatically within 3 minutes.");
...
async function chooseElectricNumber(page) {
  const configuredElectricNumber = (process.env.HVCS_ELECTRIC_NUMBER || "").trim();

  if (!configuredElectricNumber) {
    const selected = await waitForManualElectricNumberSelection(page);
...
async function clickElectricNumber(page, electricNumber) {
  const escaped = electricNumber.replace(/"/g, '\\"');
  const exactText = page.getByText(electricNumber, { exact: false });
  if ((await exactText.count().catch(() => 0)) > 0) {
    await ensureVisibleAndClick(exactText, `電號 ${electricNumber}`);
    return true;
  }

  const row = page.locator(`tr:has-text("${escaped}")`).first();
  if ((await row.count().catch(() => 0)) > 0) {
    const link = row.getByRole("link").first();
    if ((await link.count().catch(() => 0)) > 0) {
      await ensureVisibleAndClick(link, `電號 ${electricNumber}`);
      return true;
    }
    await ensureVisibleAndClick(row, `電號 ${electricNumber}`);
    return true;
  }
...
async function waitForManualElectricNumberSelection(page) {
  const startUrl = page.url();
  const startText = await page.locator("body").innerText().catch(() => "");
  ...
  if (!(await waitForElectricNumberTransition(page, startUrl, startText))) {
    log("No page change was detected automatically within 2 minutes.");
...
async function navigateToDashboard(page) {
  ...
  await chooseElectricNumber(page);
...
async function main() {
  ...
  try {
    ...
    const definitionListPairs = await collectDefinitionLists(page);
    const metricCandidates = dedupeMetrics(await collectMetricCandidates(page));
    ...
    await writeArtifacts(page, payload);
